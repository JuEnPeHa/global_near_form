...
impl Contract {
    pub fn get_number_answers(&self) -> U64 {
        U64(
            self.accounts_already_answered_nit.len() as u64 + 
            self.accounts_already_answered_nbd.len() as u64)
    }
    ...
    pub fn get_answer_by_form_id_nit(
        &self, 
        form_id: FormId
    ) -> Option<AnswerNIT> {
    ...
    pub fn get_student_account(
        &self,
        student_account: AccountId
    ) -> Option<StudentAccount> {
